...
class NeuralNet:
    ...
    def preprocess(self, X):
        ...
        for i in range(temp.shape[1]):
            temp[:,i] = le.fit_transform(temp[:,i].astype(str))
        temp_df = pd.DataFrame.from_records(temp)

        # Standardization
        temp_df = preprocessing.scale(temp_df)
        #scaler = preprocessing.StandardScaler().fit(raw_input)
        #result = scaler.transform(raw_input)

        # Normalization
        temp_df = preprocessing.normalize(temp_df, 'l2')
