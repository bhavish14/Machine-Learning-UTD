...
## TODO: Step 3
def entropy(rows):
    #returns a dict of distinct class labels and their count.
    counts = class_counts(rows)
    entropy = 0

    for index in counts:
        temp_factor = counts[index] / float(len(rows))
        entropy+= -1 *  temp_factor * math.log(temp_factor, 2)
    return entropy
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.id = id
        self.depth = depth
        #print ("func rows:", class_counts(rows).items())
        keys = class_counts(rows).keys()
        values = class_counts(rows).values()

        #(predicted_class_label, count),  = class_counts(rows).items()
        keys = list(keys)
        values = list(values)
        values_max = values.index(max(values))
        print (values_max)
        self.class_label = keys[values_max]
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print (spacing + " Class: " + str(node.class_label) + ", Predict:", str(node.predictions), ", Node.depth:", node.depth, ", Node.Id:", node.id)
        return

    # Print the question at this node
    print (spacing, str([node.question, ", Node.depth:", node.depth, ", Node.Id:", node.id]) )
...
def computeAccuracy(rows, node):
    ...
    for row in rows:
        ...
        keys = list(predicted_label.keys())
        label = list([row[-1]])
        if keys == label:
            numAccurate += 1
